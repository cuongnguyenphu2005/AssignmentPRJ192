...
public class AddBookToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String quantityChoosen = request.getParameter("intQuantityChoosen");
        String lastSearchValue = request.getParameter("txtLastSearch");

        try {
            int quantity = Integer.parseInt(quantityChoosen);
            if (quantity > 0) {
                //2. New DAO
                BookDAO dao = new BookDAO();
                //2.1 Call method from DAO
                HttpSession session = request.getSession();
                List<BookDTO> oldCart = (List<BookDTO>) session.getAttribute("ITEM_IN_CART");
                if (oldCart != null) {
                    dao.setBookItem(oldCart); // bạn cần viết thêm setter
                }
                dao.addToCart(bookId, quantity);

                List<BookDTO> result = dao.getBookItem();
                //3. Process
                session.setAttribute("ITEM_IN_CART", result);
                url = "DispatchServlet"
                        + "?txtSearchProduct=" + lastSearchValue
